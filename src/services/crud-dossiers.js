 ...
 */
export async function lireTout(uid,triSelec=['datemodif','desc']) {
  const dossiers = [];
  /************************************************************** Exercice #5 : question A **************************/
  // Modifier très légèrement la ligne suivante
  const reponse = await firestore.collection(utilRef).doc(uid).collection(dossRef).orderBy(triSelec[0],triSelec[1]).get();
  reponse.forEach(
    doc => {
